...
public class ListBoundedIntSet implements BoundedIntSet {
  ...
  * @return an int giving the maximum number of values the set can contain
  ...
  private void pullToHead (final ListNode prev, final ListNode curr) {
    // pulls node curr to the head of the list,
  ...
  public boolean contains (final int value) {
    ...
    // when the node is found, pull it to the head of the list
    ListNode prev = null;
    ListNode curr = head;
    while (true) {
  ...
  public int add (final int value) {
	...
	while(true) {
	  if (curr.value == value) { // value is present 
	    pullToHead(prev, curr);
		return 0;
	  } else if (curr.value < 0) { // value not present but found negative number 
		curr.value = value;
		pullToHead(prev, curr);
		return 1;
	  }
		
	  // call getNext() as late as possible in execution
	  ListNode next = curr.getNext();
	  
	  if (next == null) { // value not present and at end of the list
		curr.value = value;
		pullToHead(prev, curr);
		return 2;
	  } 
  ...
  /**
   * @return a string depicting the current state of the set
   */
  @Override
